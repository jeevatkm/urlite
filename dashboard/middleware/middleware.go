...
func AppInfo(ctx *Context) func(*web.C, http.Handler) http.Handler {
	return func(c *web.C, h http.Handler) http.Handler {
		fn := func(w http.ResponseWriter, r *http.Request) {
			c.Env["Config"] = ctx.Config
			c.Env["AppVersion"] = ctx.Version
			c.Env["DomainCount"] = len(ctx.Domains)
...
func Database(ctx *Context) func(*web.C, http.Handler) http.Handler {
	return func(c *web.C, h http.Handler) http.Handler {
		fn := func(w http.ResponseWriter, r *http.Request) {
			dbs := ctx.DBSession.Clone()
			defer dbs.Close()
			c.Env["DB"] = dbs.DB(ctx.Config.DB.Name)
...
func Session(ctx *Context) func(*web.C, http.Handler) http.Handler {
	return func(c *web.C, h http.Handler) http.Handler {
		fn := func(w http.ResponseWriter, r *http.Request) {
			// No session for Path /api/* since it's goes with bearer token
			if !isRoute(r, ctx.NoSessionRoute...) {
				session, err := ctx.Store.Get(r, "urlite-session")
...
func Auth(ctx *Context) func(*web.C, http.Handler) http.Handler {
	return func(c *web.C, h http.Handler) http.Handler {
		fn := func(w http.ResponseWriter, r *http.Request) {
			if isRoute(r, "/api") {
				c.Env["ReqMode"] = "API"
			} else if !isRoute(r, ctx.Config.Security.PublicPath...) {
				c.Env["ReqMode"] = "WEB"
...
func AdminAuth(ctx *Context) func(*web.C, http.Handler) http.Handler {
	return func(c *web.C, h http.Handler) http.Handler {
		fn := func(w http.ResponseWriter, r *http.Request) {
...
func ApiAuth(ctx *Context) func(*web.C, http.Handler) http.Handler {
	return func(c *web.C, h http.Handler) http.Handler {
		fn := func(w http.ResponseWriter, r *http.Request) {
