...
func Urlite(ctx *Context, c web.C, r *http.Request) *Response {
	...
	return HandleUrlite(ctx, c, r)
}

func HandleUrlite(ctx *Context, c web.C, r *http.Request) *Response {
	...
	domain, err := ctx.GetDomainDetail(liteReq.Domain)
	if err != nil {
	...
	if len(urliteId) > 0 { // Custom name mode
		...
		ctx.IncDomainCustomLink(domain.Name)
	} else { // Hash generate mode
		log.Debug("Hash generate mode")

		linkNum := ctx.GetDomainLinkNum(domain.Name)
		turliteId, err := ctx.GetUrliteID(domain.Name, linkNum)
		if err != nil {
