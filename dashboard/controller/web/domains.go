...
func Domains(ctx *Context, c web.C, r *http.Request) *Response {
	domains, _ := model.GetAllDomain(ctx.DB(&c))
	AddData(c, Data{
		"IsDomains": true,
		"Domains":   domains,
	})

	content, err := ctx.Parse("domains", c.Env)
	code := CheckError(err)
...
func DomainsValidate(ctx *Context, c web.C, r *http.Request) *Response {
	...
	if len(dTrack) > 0 {
		res := ctx.CheckDomainTrackName(dTrack)
		if res {
			...
			for i := 1; i <= 20; i++ {
				...
				res = ctx.CheckDomainTrackName(nDTrack)
				if !res {
					data := Data{
						"id":             "validation",
...
func DomainsPost(ctx *Context, c web.C, r *http.Request) *Response {
	dName := strings.TrimSpace(r.FormValue("dName"))
	if _, ok := ctx.Domains[dName]; ok {
