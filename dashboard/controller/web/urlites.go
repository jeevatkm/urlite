...
func Urlites(ctx *Context, c web.C, r *http.Request) *Response {
	u := GetUser(c)
	...
	content, err := ctx.Parse("urlites", c.Env)
	code := CheckError(err)
...
func UrlitesData(ctx *Context, c web.C, r *http.Request) *Response {
	...
	d, err := ctx.GetDomainDetail(dName)
	if err != nil {
